...
def _build_store_knowledge(tenant: TenantConfig) -> Knowledge:
    """Cria base de conhecimento de produtos e politicas da loja."""
    db = PostgresDb(
        id=f"whatsapp-knowledge-db-{tenant.tenant_id}",
        db_url=tenant.db_url,
    )
    vector_db = PgVector(
        db_url=tenant.db_url,
        table_name=f"jarvis_store_vectors_{tenant.tenant_id}",
    )
    return Knowledge(
        name=f"Conhecimento da Loja — {tenant.store_name}",
        description=(
            "Informacoes sobre produtos, precos, politicas de entrega, trocas, "
            "garantias e diferenciais da loja para uso no atendimento ao cliente."
        ),
        contents_db=db,
        vector_db=vector_db,
    )
...
def get_whatsapp_agent(tenant: TenantConfig) -> Agent:
    ...
    db = PostgresDb(
        id=f"whatsapp-agent-db-{tenant.tenant_id}",
        db_url=tenant.db_url,
    )
    knowledge = _build_store_knowledge(tenant)
    tools: List[Any] = []
    ...
    return Agent(
        ...
        instructions=[
            ...
            "Consulte sua base de conhecimento para informacoes precisas sobre produtos da loja.",
